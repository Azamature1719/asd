...
LINE_DELIMETER = '\n'
WORD_DELIMETER = ' '
LINE_NUM = 0           # index element = (line_num, word_position)    
WORD_POSITION = 1      # constants for identification an index element  
...
def input_strings():
    ...
    for i in range(len(characters)):
        if(characters[i] == LINE_DELIMETER):
...
def circular_shift():
    ...
    index.append((line_id, char_id))

    while(char_id < len(characters)):
        if(characters[char_id] == WORD_DELIMETER):  # word is found
            index.append((line_id, char_id + 1))
        if(characters[char_id] == LINE_DELIMETER): # new line is found
            line_id += 1
            index.append((line_id, char_id + 1))
        char_id += 1
...
def create_shift_from_index(_index, _id):
    static_part_start = _index[_id][WORD_POSITION]
    static_part_end = id_end_line[_index[_id][LINE_NUM]]

    shift_part_start = id_start_line[_index[_id][LINE_NUM]]
    shift_part_end = _index[_id][WORD_POSITION]
...
def alphabetizing():
    ...
    characters = characters.replace(LINE_DELIMETER, WORD_DELIMETER)
